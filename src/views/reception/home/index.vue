    ...
    <div class="carousel-container">
      ...
      >
        <el-carousel-item
          ...
          :key="item"
        >
          <el-image fit="cover" :src="baseUrl + item" class="rotate-img" />
        </el-carousel-item>
      </el-carousel>
    </div>
    <div class="list">
      <h3 class="title">推荐景点</h3>
...
export default {
  ...
  methods: {
    ...
    async getLsit() {
      ...
      this.loading = true
      this.rotateList = []
      await service.post('/carouselImage/select').then(res => {
        if (res.data) {
          this.rotateList = res.data || []
        }
      }).catch(error => {
        console.log(error)
        this.loading = false
      ...
      // 展示列表
      this.cardList = []
      await service.post('/scenicSpot/recommend').then(res => {
        console.log('res: ', res)
        this.loading = false
        if (res.data) {
